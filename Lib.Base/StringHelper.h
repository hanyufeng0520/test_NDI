...
{
	int len = MultiByteToWideChar(CP_UTF8, 0, strUTF8.c_str(), -1, nullptr, 0) - 1;
	std::wstring result;
	result.resize(len, 0);
...
{
	int len = WideCharToMultiByte(CP_UTF8, 0, strUnicode.c_str(), -1, nullptr, 0, nullptr, nullptr) - 1;
	std::string result;
	result.resize(len, 0);
...
{
	int len = MultiByteToWideChar(CP_ACP, 0, str.c_str(), -1, nullptr, 0) - 1;
	std::wstring result;
	result.resize(len, 0);
...
{
	int len = WideCharToMultiByte(CP_ACP, 0, wstr.c_str(), -1, nullptr, 0, nullptr, nullptr) - 1;
	std::string result;
	result.resize(len, 0);
